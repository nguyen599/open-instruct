...
async def update_llm_engine_weights_async(llm_engine: Any, update_info: WeightUpdateRPCArgs) -> None:
    request = WeightTransferUpdateRequest(**update_info)
    start_weight_update = getattr(llm_engine, "start_weight_update", None)
    finish_weight_update = getattr(llm_engine, "finish_weight_update", None)
...
class LLMRayActor:
    ...
    def update_weights(self, update_info: WeightUpdateRPCArgs, model_step: int | None = None) -> None:
        ...
        while not self.inflight_updates and len(self.active_tasks) > 0:
            ...
            time.sleep(DRAIN_ACTIVE_TASKS_SLEEP_S)
        self._run_async(update_llm_engine_weights_async(self.llm_engine, update_info))
